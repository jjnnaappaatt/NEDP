...
const errMsg = (e?: string) =>
  e === "not_contact" ? "ต้องเป็นผู้รับผิดชอบโครงการ"
    : e === "not_enabled" ? "ยังไม่ได้เปิดใช้งานการนำเข้าข้อมูลรายบุคคล"
      : e === "no_questionnaire" ? "โครงการนี้ยังไม่ได้กำหนดแบบสอบถาม"
        : e === "missing_columns" ? "ไฟล์ไม่มีคอลัมน์ จังหวัด/อำเภอ/ตำบล"
          : e === "empty" ? "ไฟล์ว่าง" : e === "no_match" ? "ไม่พบพื้นที่ที่ตรงกับโครงการ" : "อัปโหลดไม่สำเร็จ";

/** Bulk per-project questionnaire intake: download the template → upload to PREVIEW → confirm to commit
 *  (scores + AAI computed server-side). Mirrors PersonsXlsxCard against the questionnaire endpoints. */
export function QuestionnaireXlsxCard({ projectId, canEdit }: { projectId: string; canEdit: boolean }) {
  ...
  const post = async (file: File, mode: "preview" | "commit") => {
    ...
    const res = await fetch("/api/upload-questionnaire", { method: "POST", body: fd });
  ...
  const preview = async (file: File) => {
    ...
    try {
      ...
      if (d.ok) setReport(d as unknown as Report); else setResult(errMsg(d.error as string | undefined));
  ...
  const commit = async () => {
    ...
    try {
      ...
      if (enrolled > 0 || assessed > 0 || d.ok) {
        ...
        setResult(`นำเข้าสำเร็จ: เพิ่มใหม่ ${enrolled} คน · บันทึกแบบสอบถาม ${assessed} รายการ${failed ? ` · ผิดพลาด ${failed}` : ""}`);
  ...
  return (
    <div className="space-y-3">
      {!canEdit && <div className="rounded-card border border-warning/40 bg-warning-bg p-2.5 text-xs text-warning-fg">ต้องเป็นผู้รับผิดชอบโครงการจึงจะนำเข้าได้</div>}
      <div className="rounded-card bg-surface-soft/60 p-4">
        <div className="mb-1 flex items-center gap-1.5 text-sm font-semibold text-ink">
          <IconFileSpreadsheet size={18} /> นำเข้าแบบสอบถามเป็นชุด (Excel)
        </div>
        <p className="mb-3 text-xs text-ink-soft">ดาวน์โหลดแบบฟอร์มตามแบบสอบถามของโครงการ กรอกออฟไลน์ อัปโหลดเพื่อ<b>ตรวจสอบก่อน</b> แล้วยืนยันบันทึก — ระบบคำนวณคะแนน + AAI ให้อัตโนมัติ</p>
        <div className="flex flex-wrap gap-2">
          <a href={`/api/template/questionnaire/${projectId}`} className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-border px-3 py-2.5 text-sm font-medium text-ink-soft hover:bg-surface">
            <IconDownload size={16} /> ดาวน์โหลดแบบฟอร์ม
          </a>
          <button onClick={() => fileRef.current?.click()} disabled={busy || !canEdit} className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-hero px-4 py-2.5 text-sm font-semibold text-[var(--on-primary)] disabled:opacity-40">
          ...
          <div className="mt-3 rounded-card border border-border bg-surface p-3 text-sm">
            ...
            {report.piiWarning && (
              <div className="mt-2 flex items-start gap-1.5 rounded border border-warning/40 bg-warning-bg p-2 text-xs text-warning-fg">
                <IconAlertTriangle size={14} className="mt-0.5 shrink-0" /> ไฟล์มีชื่อ–สกุลจริง — ระบบจะจัดเก็บแบบเข้ารหัส ไม่แสดงบนแดชบอร์ด
              </div>
            )}
            {report.sample.some((s) => s.errors.length > 0) && (
              <div className="mt-2 max-h-40 overflow-auto text-xs">
                {report.sample.filter((s) => s.errors.length > 0).map((s) => (
                  <div key={s.rowNo} className="border-t border-border/60 py-1 text-ink-soft"><span className="font-medium text-warning-fg">แถว {s.rowNo}</span> ({s.tambon}): {s.errors.join(" · ")}</div>
                ))}
              </div>
            )}
            {report.truncated && <p className="mt-2 text-xs text-warning-fg">ไฟล์ใหญ่เกินกำหนด — โปรดแบ่งไฟล์</p>}
            <div className="mt-3 flex gap-2">
              <button onClick={commit} disabled={busy || report.valid === 0} className="rounded-lg bg-hero px-4 py-2 text-sm font-semibold text-[var(--on-primary)] disabled:opacity-40">{busy ? "กำลังนำเข้า…" : `ยืนยันนำเข้า ${report.valid} คน`}</button>
              <button onClick={() => { setReport(null); pending.current = null; }} className="rounded-lg border border-border px-4 py-2 text-sm text-ink-soft hover:bg-surface">ยกเลิก</button>
            </div>
          </div>
        )}
      </div>
