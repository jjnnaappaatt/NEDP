...
/** Inbound LINE webhook (replaces Railway's /line/webhook). Verifies X-Line-Signature over the RAW body,
 *  then dispatches. Always returns 200 to LINE (even on handler errors) so LINE doesn't retry-storm; each
 *  event is isolated inside handleWebhook. */
...
/** LINE's "Verify" button in the console sends a GET/empty check to some setups; respond 200. */
export function GET() {
  return NextResponse.json({ ok: true });
}
